...
// Replace StatPill
const statPillReplacement = `function StatPill({
  label,
  value,
  icon: Icon,
  color,
...
code = code.replace(/function StatPill\(\{[\s\S]*?\}\) \{\s*return \([\s\S]*?<\/Link>\s*\)\s*\}/, statPillReplacement)


// Replace the Header and the grid
const headerRegex = /\{\/\* Header \*\/\}\s*<div className="app-panel flex items-center justify-between rounded-\[30px\] px-6 py-6 sm:px-8">[\s\S]*?<\/div>/
...
// Replace Stats
const statsRegex = /\{\/\* Stats \*\/\}\s*\{loading \? \([\s\S]*?\) : \([\s\S]*?<div className="grid grid-cols-2 gap-4 lg:grid-cols-3">[\s\S]*?<StatPill[\s\S]*?<\/div>\s*\)\}/
...
// Replace Recent contacts app-panel with premium style
code = code.replace(
...
code = code.replace(
  ...
  '<li key={c.id} className="group flex items-center gap-4 px-6 py-4 transition-all hover:bg-slate-50/50">'
)

// Replace CRM Tools app-panel with premium style
code = code.replace(
  '<section className="app-panel overflow-hidden rounded-[28px]">',
  '<section className="relative overflow-hidden rounded-[32px] border border-white/80 bg-white/60 shadow-[0_4px_20px_-8px_rgba(15,23,42,0.06)] backdrop-blur-xl">'
)
...
code = code.replace(
  /<li key=\{link\.href\}>/g,
  '<li key={link.href} className="group transition-all hover:bg-slate-50/50">'
)
code = code.replace(
  /className="flex items-center gap-4 px-5 py-4"/g,
  'className="flex items-center gap-5 px-6 py-4.5 transition-all group-hover:pl-7"'
)

// In the CRM tools
code = code.replace(
  /<Icon className="h-5 w-5 text-gray-400 shrink-0" \/>/g,
  '<Icon className="h-5 w-5 text-slate-400 shrink-0 transition-colors group-hover:text-violet-500" />'
)
code = code.replace(
  /<ArrowRight className="h-4 w-4 text-gray-300" \/>/g,
