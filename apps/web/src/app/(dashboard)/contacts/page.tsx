...
import { Plus, Search, Download, Trash2, Users, AlertTriangle } from 'lucide-react'
...
export default function ContactsPage(): JSX.Element {
  ...
  const loadContacts = useCallback(async (p = 1, s = search, stage = stageFilter) => {
    setLoading(true)
    setError(null)
    try {
  ...
  const handleStageFilter = useCallback((stage: string) => {
  ...
  const handleBulkStageChange = useCallback(async () => {
    ...
    try {
      const token = await getAccessToken()
      await Promise.all([...selectedIds].map(id => apiPatch(`/contacts/${id}/stage`, { stage: bulkStage }, token)))
      setSelectedIds(new Set())
      setBulkStage('')
  ...
  const handleBulkDelete = useCallback(async () => {
    setConfirmDialog({
      ...
      onConfirm: async () => {
        try {
          const token = await getAccessToken()
          await Promise.all([...selectedIds].map(id => apiDelete(`/contacts/${id}`, token)))
          setSelectedIds(new Set())
          void loadContacts(page)
        } catch {
    ...
    <div className="space-y-6">
      ...
      <div className="overflow-x-auto rounded-xl border border-gray-200 bg-white shadow-sm">
        {loading ? (
          ...
          <div className="flex flex-col items-center justify-center py-16 text-center">
            ...
            {total === 0 && !search && stageFilter === 'ALL' ? (
              <>
          ...
          <table className="w-full text-sm">
            <thead className="bg-gray-50">
              <tr>
                ...
                <th className="px-4 py-3 text-left text-xs font-semibold uppercase tracking-wider text-gray-500 hidden md:table-cell">Company</th>
                <th className="px-4 py-3 text-left text-xs font-semibold uppercase tracking-wider text-gray-500 hidden lg:table-cell">Source Card</th>
                <th className="px-4 py-3 text-left text-xs font-semibold uppercase tracking-wider text-gray-500 hidden lg:table-cell">Date Added</th>
            ...
            <tbody className="divide-y divide-gray-100">
              {contacts.map(contact => (
                ...
                >
                  ...
                  <td className="px-4 py-3 text-gray-500 hidden md:table-cell" onClick={() => setDrawerContactId(contact.id)}>
                    {contact.company ?? '—'}
                  </td>
                  <td className="px-4 py-3 hidden lg:table-cell" onClick={() => setDrawerContactId(contact.id)}>
                    {contact.sourceCard ? (
                  ...
                  <td className="px-4 py-3 text-gray-500 hidden lg:table-cell" onClick={() => setDrawerContactId(contact.id)}>
                    {new Date(contact.createdAt).toLocaleDateString()}
                  </td>
                  <td className="px-4 py-3" onClick={() => setDrawerContactId(contact.id)}>
                    <span className={`rounded-full px-2.5 py-0.5 text-xs font-medium ${STAGE_BADGE[(contact.crmPipeline?.stage as Stage) ?? 'NEW'] ?? ''}`}>
      ...
      {showAddModal && (
        <AddContactModal
          ...
          onCreated={() => { setShowAddModal(false); void loadContacts(1) }}
        />
      )}
...
function AddContactModal({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const token = await getAccessToken()
      await apiPost<ContactDetail>('/contacts', { name, email, phone, company, sourceCardId: sourceCardId || undefined, stage }, token)
      onCreated()
    } catch (err) {
  ...
  return (
    <>
      ...
      <div className="fixed inset-x-4 top-1/2 z-50 w-full max-w-md -translate-y-1/2 rounded-xl bg-white p-6 shadow-2xl sm:inset-x-auto sm:left-1/2 sm:-translate-x-1/2">
        <h2 className="mb-4 text-lg font-semibold text-gray-900">Add Contact</h2>
        {error && <p className="mb-3 text-sm text-red-600">{error}</p>}
        <form onSubmit={e => void handleSubmit(e)} className="space-y-3">
          <input required placeholder="Full Name *" value={name} onChange={e => setName(e.target.value)} className="w-full rounded-lg border border-gray-300 px-3 py-2 text-sm focus:border-indigo-500 focus:outline-none" />
          <input type="email" placeholder="Email *" value={email} onChange={e => setEmail(e.target.value)} className="w-full rounded-lg border border-gray-300 px-3 py-2 text-sm focus:border-indigo-500 focus:outline-none" />
          <input placeholder="Phone" value={phone} onChange={e => setPhone(e.target.value)} className="w-full rounded-lg border border-gray-300 px-3 py-2 text-sm focus:border-indigo-500 focus:outline-none" />
          <input placeholder="Company" value={company} onChange={e => setCompany(e.target.value)} className="w-full rounded-lg border border-gray-300 px-3 py-2 text-sm focus:border-indigo-500 focus:outline-none" />
          <select value={sourceCardId} onChange={e => setSourceCardId(e.target.value)} className="w-full rounded-lg border border-gray-300 px-3 py-2 text-sm focus:border-indigo-500 focus:outline-none">
            <option value="">No source card</option>
            {cards.map(c => <option key={c.id} value={c.id}>/{c.handle}</option>)}
          </select>
          <select value={stage} onChange={e => setStage(e.target.value as Stage)} className="w-full rounded-lg border border-gray-300 px-3 py-2 text-sm focus:border-indigo-500 focus:outline-none">
            {STAGES.map(s => <option key={s} value={s}>{s}</option>)}
          </select>
