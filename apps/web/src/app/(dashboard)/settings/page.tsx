...
const TIMEZONE_VALUE_SET = new Set(TIMEZONE_OPTIONS.map((timezone) => timezone.value))
...
export default function SettingsPage(): JSX.Element {
  ...
  const [profileFieldErrors, setProfileFieldErrors] = useState<
    Partial<Record<'name' | 'country' | 'timezone', string>>
  >({})
  ...
  const handleProfileSave = useCallback(async () => {
    ...
    const nextFieldErrors: Partial<Record<'name' | 'country' | 'timezone', string>> = {}

    if (trimmedName.length > 200) {
      nextFieldErrors.name = 'Full name must be 200 characters or less.'
    }

    if (normalizedCountry && !COUNTRY_CODE_SET.has(normalizedCountry)) {
      nextFieldErrors.country = 'Select a valid country.'
    }

    if (normalizedTimezone && !TIMEZONE_VALUE_SET.has(normalizedTimezone)) {
      nextFieldErrors.timezone = 'Select a valid timezone.'
    }
